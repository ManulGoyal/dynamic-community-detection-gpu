...
__global__ void computeMoveGlobal2(int V, int *vertices, int *primes, device_structures deviceStructures, int *hashCommunity, float *hashWeight) {
	int verticesPerBlock = blockDim.y;
	int vertexIndex = blockIdx.x * verticesPerBlock + threadIdx.y;
	if (vertexIndex < V) {
		extern __shared__ int s[];
...
bool optimiseModularity(float minGain, device_structures& deviceStructures, host_structures& hostStructures) {
    ...
    if (verticesInLastBucket > 0) {
		/* Manul change start */
		int* partition_h = (int*) malloc(verticesInLastBucket * sizeof(int));
		int* primes_h = (int*) malloc((verticesInLastBucket + 1) * sizeof(int));
		HANDLE_ERROR(cudaMemcpy(partition_h, partition, verticesInLastBucket * sizeof(int), cudaMemcpyDeviceToHost));
		primes_h[0] = 0;
		for(int vi = 0; vi < verticesInLastBucket; vi++) {
			int v = partition_h[vi];
			int degv = hostStructures.edgesIndex[v+1] - hostStructures.edgesIndex[v];
			primes_h[vi + 1] = primes_h[vi] + getPrime(degv * 1.5);
		}
		
		HANDLE_ERROR(cudaMalloc((void**)&primes_d, (verticesInLastBucket + 1) * sizeof(int)));
		...
		HANDLE_ERROR(cudaMalloc((void**)&hashCommunity, primes_h[verticesInLastBucket] * sizeof(int)));		// Manul change
...
bool optimiseModularityUsingVertexSubset(float minGain, device_structures& deviceStructures, host_structures& hostStructures, int R_size) {
	...
	while (totalGain >= minGain) {
		...
		for(int bucketNum= 0; bucketNum < bucketsSize - 2; bucketNum++) {
			...
			int prime = primes[bucketNum];
			auto predicate = isInBucket(buckets[bucketNum], buckets[bucketNum + 1], hostStructures.edgesIndex);
			deviceVerticesEnd = thrust::partition(thrust::device, partition, partition + nodeEvalCount, predicate);			//
			int verticesInBucket = thrust::distance(partition, deviceVerticesEnd);
			if (verticesInBucket > 0) {
                int sharedMemSize =
                        blockDimension.y * prime * (sizeof(float) + sizeof(int)) + blockDimension.y * sizeof(float);
...
void printOriginalToCommunity(device_structures& deviceStructures, host_structures& hostStructures) {
	...
	printf("Total comm's = %d\n", hostStructures.V);
	for (int community = 0; community < hostStructures.V; community++) {
		printf("Vertices in Comm %d:", community + 1);
